...
import { Match, Game as GameType } from '../interfaces';
...
const Game = ({ match, game, index, spoil: defaultSpoil }: GameProps) => {
  ...
  if (typeof match.team1 === 'string' || typeof match.team2 === 'string') {
    return null;
  }
  ...
  return (
    <div className='game-card'>
      <div className='game-header' style={{ 'borderRadius': `${headerRadius}px ${headerRadius}px 0px 0px` }}>
        ...
        <p>{game.status === "finished" ? getFormattedTime(game.length || 0) : ""}</p>
      </div>
