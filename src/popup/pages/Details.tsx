...
import { Match, CalendarFilters } from '../interfaces';
...
const Details = () => {
  ...
  const [category, setCategory] = useState<'overview' | 'graph'>('overview');
  ...
  useEffect(() => {
    ...
    fetchAPI(`details/${match.id}`)
      ...
      .then(data => {
        ...
        Object.values(data.games).forEach((game: any) => {
          ...
          players.forEach((player: any) => {
            if (match.game === 'League of Legends') {
              ...
              player.summoners.forEach((summoner: any) => imagesToLoad.push(summoner.icon));
              player.items.forEach((item: any) => imagesToLoad.push(item.icon));
          ...
          Object.values(game.teams).forEach((team: any) => {
  ...
  if (noData) {
    ...
    const game = games[currentGame] as any;
    const players = Object.values(match.game === 'League of Legends' ? game.players : game.members) as any[];

    const teams = Object.values(game.teams) as any[];

    teams.forEach((team: any) => {
      team.players = players.filter((player: any) => player.team === team.name);
    });

    const Items = (items: any[], trinket: any, side?: string) => {
    ...
    const KdaSVG = ({ className }: { className?: string }) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className={className}>
      <title>Kills / Deaths / Assists</title>
    ...
    const GoldSVG = () => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
      <title>Golds</title>
    ...
    let maxDamage = Math.max(...players.map((p: any) => p.damage));

    const PlayerDamageGraph = ({ p, blue }: { p: any; blue: boolean }) => {
      const percentage = p.damage / maxDamage * 100;
      return (
        <div className='player'>
          <img style={{ border: `2px solid ${blue ? '#0a96aa' : '#b71d36'}` }} src={p.champion.icon} alt={p.champion.name} title={p.name} />
    ...
    const Rounds = ({ rounds }: { rounds: any[] }) => {
      return (
                        ...
                        <div key={index} className='player'>
                          <div className='item-1'>
                            ...
                            <div className='summoners'>
                              {player.summoners.map((summoner: any, index: number) => (
                      ...
                      <div className="team" key={index}>
                        ...
                        <div className='players'>
                          {team.players.sort((a: any, b: any) => {
                            return b.rating[sideFilter] - a.rating[sideFilter];
                            ...
                            return (
                              <div className='player' key={index} >
                                <div className='item-1'>
                                  ...
                                  <div>
                                    <p className='name'>{player.name}</p>
                                    <p className='acronym'>{teams.find((t: any) => t.name === player.team)?.acronym}</p>
                                  </div>
                                </div>
